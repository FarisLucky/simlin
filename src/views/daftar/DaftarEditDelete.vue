<template>
    <b-modal
        v-model="modal"
        id="modal-standard"
        :title="modalTitle"
        title-class="font-18"
        hide-footer
    >
        <div class="mb-1">
            <h5>
                Apakah ingin menghapus data
                <strong class="text-info">{{ modalForm?.nama }}</strong>
            </h5>
        </div>
        <hr class="mb-2" />
        <div class="mb-1">
            <BButton variant="danger" @click.prevent="onDelete" class="w-100">
                <i class="bx bx-trash"></i>
                Hapus
            </BButton>
        </div>
    </b-modal>
</template>
<script>
import { JENIS } from "@/helpers/utils";
import { pinjamService } from "@/services/PinjamService";
import { transLinenDetailService } from "@/services/TransLinenDetailService";
import { spinnerMethods, toastMethods } from "@/state/helpers";

export default {
    props: ["jenisTrans"],
    data() {
        return {
            modalTitle: "",
            modalUnit: "",
            modalForm: {},
            modal: false,
        };
    },
    methods: {
        ...spinnerMethods,
        ...toastMethods,
        showModal() {
            this.modal = true;
        },
        hideModal() {
            this.modal = false;
        },
        async onDelete() {
            this.show();
            let connect;
            if (this.jenisTrans === JENIS.LINEN) {
                connect = await transLinenDetailService.delete(
                    this.modalForm.id
                );
            } else {
                connect = await pinjamService.delete(this.modalForm.id);
            }
            const [err] = connect;
            if (err) {
                this.toastError({
                    title: "Gagal",
                    msg: err.response?.data?.errors,
                });
                this.hide();
                return;
            }
            this.hide();
            this.$emit("fetch");
            this.hideModal();
        },
    },
};
</script>
<style lang="">
</style>